...
**RSS Feed:**
- Generated by `python scripts/generate_feed.py`, merging blog posts (from
  `blog-manifest.json`), Pokemon chapters (from `pokemon/<n>/index.html`), and
  Rationally Writing episodes (from `rationally-writing/<n>/index.html`)
- Written to both `feed.xml` (canonical, `/feed.xml`) and `feed/index.html`
  (legacy WordPress URL `/feed/` that older subscribers still use)
- Regenerate and commit both files whenever a blog post, chapter, or episode is
  added, or the feed goes stale
- New Pokemon chapters need their date added to `CHAPTER_DATES`, and new episodes
  need their date added to `EPISODE_DATES`, in `generate_feed.py`
- The Rationally Writing podcast also has its own Anchor audio feed
  (`https://anchor.fm/s/872330c/podcast/rss`) for podcast apps
...
5. Regenerate the RSS feed: `python scripts/generate_feed.py` (commit `feed.xml`
   and `feed/index.html`)
...
6. Add the chapter's date to `CHAPTER_DATES` in `scripts/generate_feed.py`, then
   run `python scripts/generate_feed.py` (commit `feed.xml` and `feed/index.html`)
